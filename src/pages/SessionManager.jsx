...
export default function SessionManager({
  ...
  const checkUserConnection = async () => {
    if (!sessionId) return;
    try {
      const res = await fetch(`http://45.113.225.69:3000/user-details/${sessionId}`);
      if (res.ok) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const sessionRes = await fetch('http://45.113.225.69:3000/create-session', { method: 'POST' });
      ...
      const qrRes = await fetch(`http://45.113.225.69:3000/generate-qrcode/${sessionData.sessionId}`);
      ...
      if (qrData.qrCode) {
        ...
        const interval = setInterval(async () => {
          const userRes = await fetch(`http://45.113.225.69:3000/user-details/${sessionData.sessionId}`);
          if (userRes.ok) {
